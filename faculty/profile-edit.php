...
$filename="../images/avatar-".$fid.".".$imageFileType;
...
if (isset($submit)){
    ...
    // if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            ...
            $query = "UPDATE `users` SET `image`='$filename' WHERE `users`.`id` = $fid";
    ...
    $query = "UPDATE `users` SET `name`='$fullname',`mobile`='$mobile',`gender`='$gender',`dob`='$doba' WHERE `users`.`id` = $fid";
    $result = mysqli_query($con,$query);
    if(!$result){ 
       $flag=0;     
    } else $flag=1;
